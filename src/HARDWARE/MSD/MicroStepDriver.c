...
static void MSD_GPIO_Config(void) 
{
    ...
    //电机脉冲输出 GPIO 初始化
	RCC_AHB1PeriphClockCmd(MSD_PULSE_GPIO_CLK,ENABLE); //使能GPIOA时钟
...
/**
  ...
  */
void MSD_PULSE_TIM_IRQHandler(void)  //中断程序
{
  // 保存下一个延时周期
...
if (TIM_GetITStatus(MSD_PULSE_TIM, TIM_IT_Update) != RESET)
{
    ...
    MSD_PULSE_TIM->CCR4=srd.step_delay >> 1;//周期的一半    //占空比
    MSD_PULSE_TIM->ARR=srd.step_delay;   //设置自动重装载值
    //如果禁止输出，电机则停止运动
